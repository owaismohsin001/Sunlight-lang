...
generate (FuncDefNode _ args expr pos) = 
    "(" ++ fun ++ ")"
    where
        fun = gen ++ " " ++ generate expr ++ unwords (map (const (", " ++ luaPos pos ++ ")")) args)
...
instance CompileMode Js where
    importGen _ = "import {SltValue, SltBool, SltNum, SltFunc, SltString, SltStruct, SltList, SltThunk, SltTuple, SltType, destructure, unwrap, listHead, listTail, baseStringify, unsafeMod, evaluate, typeOf, ovTypeOf, unsafeWrite} from './SltRuntime.ts'\n;"
